...
def tl_mul_relu_1d(A, B, BLOCK_N: int):
    ...
    with T.Kernel(N // BLOCK_N, threads=256) as bx:
        idx = bx * BLOCK_N  
        for i in T.Parallel(BLOCK_N):
            result = A[idx + i] * B[idx + i]
            C[idx + i] = T.if_then_else(result > 0, result, 0)
...
def tl_mul_relu_1d_mem(A, B, BLOCK_N: int):
    ...
    with T.Kernel(N // BLOCK_N, threads=256) as (bx,):
        for i in T.Parallel(BLOCK_N):  
            A_reg = T.alloc_fragment((BLOCK_N,), dtype)
            B_reg = T.alloc_fragment((BLOCK_N,), dtype)
            C_reg = T.alloc_fragment((BLOCK_N,), dtype)

            base_idx = bx * BLOCK_N  

            T.copy(A[base_idx + i], A_reg[i])
            T.copy(B[base_idx + i], B_reg[i])
            
            result = A_reg[i] * B_reg[i]
            C_reg[i] = T.if_then_else(result > 0, result, 0)

            T.copy(C_reg[i], C[base_idx + i])
