...
def tl_outer_add(A, B, BLOCK_N: int, BLOCK_M: int):
    N, M = T.const("N, M")      # eager模式下，在runtime通过A,B的shape推导N, M
    dtype = T.float16
    A: T.Tensor((N,), dtype)    # 给tilelang编译器提示形状和dtype
    B: T.Tensor((M,), dtype)
    C = T.empty((N, M), dtype)
    ...
    with T.Kernel(N // BLOCK_N, M // BLOCK_M, threads=256) as (bx, by):
        ...
        T.copy(A[bx_start], A_reg)
        T.copy(B[by_start], B_reg)

        for i, j in T.Parallel(BLOCK_N, BLOCK_M):
            C_reg[i, j] = A_reg[i] + B_reg[j]
            
        T.copy(C_reg, C[bx_start : bx_start+BLOCK_N, by_start : by_start+BLOCK_M])
