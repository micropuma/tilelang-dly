...
def tl_reduce_sum(A, BLOCK_N: int, BLOCK_M: int):
    ...
    with T.Kernel(N // BLOCK_N, threads=256) as (bx,):  
        ...
        for col in T.Serial(M // BLOCK_M):
            col_start = col * BLOCK_M  
            T.copy(A[row_start : row_start + BLOCK_N, col_start : col_start + BLOCK_M], A_reg)
            T.reduce_sum(A_reg, B_reg, dim=1, clear=False)
        
        T.copy(B_reg, B[row_start : row_start + BLOCK_N])
